...
exports.AdminSuKienService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("typeorm");
let AdminSuKienService = class AdminSuKienService {
    ...
    `;
            if (search) {
                danhSach = await this.dataSource.query(`${baseSelect} WHERE sk.ten_su_kien ILIKE $1 OR sk.dia_diem ILIKE $1 GROUP BY sk.id ORDER BY sk.ngay_to_chuc DESC LIMIT $2 OFFSET $3`, [`%${search}%`, limitNum, offset]);
    ...
    `;
            ...
            const kqThongKe = await this.dataSource.query(`SELECT COUNT(*) AS tong, COUNT(*) FILTER (WHERE trang_thai = 'xac_nhan') AS xac_nhan,
    ...
    async themAdminSuKien(body) {
        ...
        if (!ten_su_kien || !ngay_to_chuc)
            return { thanh_cong: false, thong_bao: 'Tên sự kiện và Ngày tổ chức không được để trống!' };
        try {
            await this.dataSource.query(`INSERT INTO su_kien (ten_su_kien, mo_ta, ngay_to_chuc, dia_diem, diem_cong) VALUES ($1, $2, $3, $4, $5)`, [ten_su_kien, mo_ta || null, ngay_to_chuc, dia_diem || null, diem_cong || null]);
    ...
    async suaAdminSuKien(id, body) {
        ...
        if (!ten_su_kien || !ngay_to_chuc)
            return { thanh_cong: false, thong_bao: 'Tên sự kiện và Ngày tổ chức không được để trống!' };
        try {
            await this.dataSource.query(`UPDATE su_kien SET ten_su_kien = $1, mo_ta = $2, ngay_to_chuc = $3, dia_diem = $4, diem_cong = $5 WHERE id = $6`, [ten_su_kien, mo_ta || null, ngay_to_chuc, dia_diem || null, diem_cong || null, id]);
    ...
    async capNhatTrangThaiDangKy(id, body) {
        ...
        if (!cacTrangThaiHopLe.includes(trang_thai))
            return { thanh_cong: false, thong_bao: 'Trạng thái không hợp lệ!' };
        try {
...
exports.AdminSuKienService = AdminSuKienService;
exports.AdminSuKienService = AdminSuKienService = __decorate([
