...
exports.AdminSinhVienService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("typeorm");
const bcrypt = require("bcrypt");
const xlsx = require("xlsx");
const BCRYPT_ROUNDS = 10;
let AdminSinhVienService = class AdminSinhVienService {
    ...
    async themAdminSinhVien(body) {
        ...
        const idVaiTroAdminSinhVien = 3;
        try {
            const kiemTraMssv = await this.dataSource.query(`SELECT id FROM sinh_vien WHERE mssv = $1`, [mssv]);
            if (kiemTraMssv.length > 0)
                return { thanh_cong: false, thong_bao: `MSSV "${mssv}" đã tồn tại trong hệ thống! Vui lòng kiểm tra lại.` };
            if (email) {
                const kiemTraEmail = await this.dataSource.query(`SELECT id FROM nguoi_dung WHERE email = $1`, [email]);
                if (kiemTraEmail.length > 0)
                    return { thanh_cong: false, thong_bao: `Email "${email}" đã được dùng bởi tài khoản khác!` };
            }
            const matKhauDaMaHoa = await bcrypt.hash(mat_khau, BCRYPT_ROUNDS);
            const taiKhoanMoi = await this.dataSource.query(`INSERT INTO nguoi_dung (email, mat_khau, vai_tro_id) VALUES ($1, $2, $3) RETURNING id`, [email, matKhauDaMaHoa, idVaiTroAdminSinhVien]);
            const nguoiDungId = taiKhoanMoi[0].id;
            await this.dataSource.query(`INSERT INTO sinh_vien (mssv, ho_ten, lop, nguoi_dung_id) VALUES ($1, $2, $3, $4)`, [mssv, ho_ten, lop, nguoiDungId]);
            return { thanh_cong: true, thong_bao: `Đã thêm sinh viên "${ho_ten}" thành công!` };
        }
        catch (error) {
            if (error.message.includes('sinh_vien_mssv_key'))
                return { thanh_cong: false, thong_bao: `MSSV "${mssv}" đã tồn tại trong hệ thống!` };
            if (error.message.includes('nguoi_dung') && error.message.includes('unique'))
                return { thanh_cong: false, thong_bao: `Email "${email}" đã được sử dụng!` };
    ...
    async xoaAdminSinhVien(id) {
        try {
            ...
            if (timSv.length === 0)
                return { thanh_cong: false, thong_bao: 'Không tìm thấy sinh viên!' };
    ...
    async suaAdminSinhVien(id, body) {
        ...
        try {
            ...
            if (mat_khau && mat_khau.trim() !== '') {
                const matKhauDaMaHoa = await bcrypt.hash(mat_khau, BCRYPT_ROUNDS);
                await this.dataSource.query(`UPDATE nguoi_dung SET email = $1, mat_khau = $2 WHERE id = (SELECT nguoi_dung_id FROM sinh_vien WHERE id = $3)`, [email, matKhauDaMaHoa, id]);
            }
            else {
                await this.dataSource.query(`UPDATE nguoi_dung SET email = $1 WHERE id = (SELECT nguoi_dung_id FROM sinh_vien WHERE id = $2)`, [email, id]);
            }
    ...
    async importAdminSinhVienTuExcel(buffer) {
        try {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                try {
                    ...
                    const idVaiTroAdminSinhVien = 3;
                    const taiKhoanMoi = await this.dataSource.query(`INSERT INTO nguoi_dung (email, mat_khau, vai_tro_id) VALUES ($1, $2, $3) RETURNING id`, [email ? email.toString() : null, mat_khau.toString(), idVaiTroAdminSinhVien]);
                    const nguoiDungId = taiKhoanMoi[0].id;
                    await this.dataSource.query(`INSERT INTO sinh_vien (mssv, ho_ten, lop, nguoi_dung_id) VALUES ($1, $2, $3, $4)`, [mssv.toString(), ho_ten.toString(), lop ? lop.toString() : null, nguoiDungId]);
                    soThanhCong++;
                }
                catch (err) {
                    soThatBai++;
...
exports.AdminSinhVienService = AdminSinhVienService;
exports.AdminSinhVienService = AdminSinhVienService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [typeorm_1.DataSource])
], AdminSinhVienService);
