let danhSachHienTai = [];
        ...
        $(document).ready(function () {
            ...
            $('#btnLuuLop').click(function () {
                ...
                let urlApi = `${API_URL}/lop-hoc-phan`;
                let phuongThuc = 'POST';

                if (idLopDangSua !== null) {
                    urlApi = `${API_URL}/lop-hoc-phan/${idLopDangSua}`;
                    phuongThuc = 'PUT';
                }
        ...
        function taiDanhSachLop(page) {
            ...
            fetch(`${API_URL}/lop-hoc-phan?page=${page}&limit=10&search=${encodeURIComponent(tuKhoa)}`)
                .then(response => response.json())
                .then(data => {
                    if (data.thanh_cong) {
        ...
        function xoaLop(id) {
            if (confirm("⚠️ Cảnh báo: Việc này sẽ xóa lớp học phần. Bạn có chắc chắn muốn xóa không?")) {
                fetch(`${API_URL}/lop-hoc-phan/${id}`, { method: 'DELETE' })
                    .then(response => response.json())
                    .then(data => {
                        if (data.thanh_cong) {
