...
@Injectable()
export class AdminLopHocPhanService {
  constructor(private dataSource: DataSource) {}

  async layDanhSachAdminLopHocPhan(page: string = '1', limit: string = '10', search: string = '') {
  ...
  async themAdminLopHocPhan(body: any) {
  ...
  async suaAdminLopHocPhan(id: string, body: any) {
    const { hoc_phan_id, giang_vien_id, hoc_ky } = body;
    try {
      await this.dataSource.query(
  ...
  async xoaAdminLopHocPhan(id: string) {
    try {
